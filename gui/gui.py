        ...
        if values['language'] == 'English':
            ...
            layout_advanced = [
                ...
                [sg.Text('Replace To NULL:', size=(15, 1)),
                 sg.Input('{}'.format(values['na_values']), key='na_values', size=(40, 1)), ],
                ...
                [sg.Text('The column on row:', size=(15, 1)),
                 sg.Input(values['header'], key='header', size=(10, 1)), sg.Text('', size=(3, 1)),],

                [sg.Checkbox('Skip Blank Rows', key='del_blank_lines', size=(15, 1),
                             default=values['del_blank_lines']),
                 sg.Checkbox('Trim Spaces', key='trim', size=(12, 1), default=values['trim']),
                 sg.Checkbox('Skip Blank Sheets', key='skip_blank_sheet', size=(12, 1),
                             default=values['skip_blank_sheet']),
                 ],

                [sg.Checkbox('Add a column is table name', key='add_tname', size=(22, 1), default=values['add_tname']),
                sg.Checkbox('Include Sub Directories', key='loop_subdir', size=(18, 1), default=values['loop_subdir']),
                 ],
            ...
            layout_advanced = [
                ...
                [sg.Text('为创建的表名添加前缀:', size=(18, 1)),
                 sg.Input(values['prefix'], key='prefix', size=(20, 1), ), ],
                [sg.Text('将数据追加到已存在的表（追加模式有效）:', size=(34, 1)),
                 sg.Input(values['tname'], key='tname', size=(20, 1), ), ],
                [sg.Text('指定列名所在行数:', size=(18, 1)), sg.Input(values['header'], key='header', size=(10, 1)),
                 ],
