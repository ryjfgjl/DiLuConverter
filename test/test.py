...
class ToOracle:
            ...
            try:
                ...
                for k, v in datasets.items():
                    ...
                    try:
                        ...
                        # 创建表
                        if values['mode1']:
                            created_table, created_sql = self.create_table(col_maxlen, tablename,values)

                        self.insert_data(dataset, tablename, values)
                        ...
                        if created_table:
                            sql = 'drop table "{}"'.format(created_table)
    ...
    # 创建表函数
    def create_table(self, col_maxlen, tablename,values):
        sql = "select 1 from all_tables where owner = '{}' and table_name = '{}'".format(values['user'].upper(),tablename)
    ...
    # 插入数据函数
    def insert_data(self, dataset, tablename,values):
        if dataset.empty:
            return
        sql = "select column_name from ALL_TAB_COLUMNS " \
              "where OWNER = '{0}' and table_name = '{1}'".format(values['user'].upper(), tablename)
